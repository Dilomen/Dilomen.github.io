...
class BST {
  ...
  insertData(root, data) {
    ...
    if (newNode.data < root.data) {
      if (root.left === null) {
        ...
        this.insertData(root.left, data);
      }
    } else {
      if (root.right === null) {
        root.right = newNode;
      } else {
        this.insertData(root.right, data);
      }
    }
  }
  ...
  lastOrder() {
    ...
    function lastOrderNode(node) {
      if (node === null) return false;
      lastOrderNode(node.left);
      lastOrderNode(node.right);
      list.push(node.data);
    }
    lastOrderNode(this.root);
    return list;
  }
}
